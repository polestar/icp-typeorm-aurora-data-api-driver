...
describe('aurora data api > simple queries', () => {
  ...
  it('should do a simple select', async () => {
    await useCleanDatabase('mysql', { logger: 'simple-console' }, async (connection: any) => {
  ...
  it('should create a table and be able to query it', async () => {
    await useCleanDatabase('mysql', { entities: [Post, Category] }, async (connection: any) => {
  ...
  it('should be able to update a date field by primary key', async () => {
    await useCleanDatabase('mysql', { entities: [Post, Category] }, async (connection: any) => {
      ...
      expect(
        Math.abs(dbPost!.updatedAt!.getTime() / 1000 - updatedAt.getTime() / 1000)
      ).toBeLessThanOrEqual(1);
    });
  });

  it('should be able to correctly deal with bulk inserts', async () => {
    await useCleanDatabase('mysql', { entities: [Category] }, async (connection: any) => {
  ...
  it('should handle date/time types', async () => {
    await useCleanDatabase('mysql', { entities: [DateEntity] }, async (connection: any) => {
  ...
  it('should handle json types', async () => {
    await useCleanDatabase('mysql', { entities: [JsonEntity] }, async (connection: any) => {
  ...
  it('should handle empty simple-array', async () => {
    await useCleanDatabase('mysql', { entities: [JsonEntity] }, async (connection: any) => {
      const simpleArray = new SimpleArrayEntity();

      simpleArray.array = [];

      const newSimpleArray = await connection.getRepository(JsonEntity).save(simpleArray);

      const loadedSimpleArray = (await connection
        .getRepository(JsonEntity)
        .findOneBy({ id: newSimpleArray.id }))!;

      // Assert
      expect(newSimpleArray).toBeTruthy();
      expect(newSimpleArray.simpleJson).toEqual(null);

      expect(loadedSimpleArray).toBeTruthy();
      expect(loadedSimpleArray.simpleJson).toEqual(null);
    });
  });
});
